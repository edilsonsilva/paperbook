...
!["Primeira modelagem no excel"](modeloconceitual-excel1.png)

#### Modelagem conceitual no excel - 1ª Normalização
!["Primeira modelagem no excel-1ª Normalização"](modeloconceitual-excel2.png)

#### Modelagem conceitual no excel - 2ª Normalização
!["Primeira modelagem no excel - 2ª Normalização"](modeloconceitual-excel3.png)

### Modelagem de dados com o diagrama de entidade relacional
#### Neste diagrama temos as cardenalidades
    - Um para Muitos
    - Um para Um
!["Segunda modelagem"](modeloconceitual-paperbook.png)
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Funcionario` (
  ...
  INDEX `fk_func_pk_func_idx` (`superior` ASC) ,
  INDEX `fk_func_pk_cargo_idx` (`idcargo` ASC) ,
  INDEX `idendereco_idx` (`idendereco` ASC) ,
  INDEX `fk_func_pk_contato_idx` (`idcontato` ASC) ,
  INDEX `fk_func_pk_usuario_idx` (`idusuario` ASC) ,
  CONSTRAINT `fk_func_pk_func`
    FOREIGN KEY (`superior`)
    REFERENCES `paperbook`.`Funcionario` (`idfuncionario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_func_pk_cargo`
    FOREIGN KEY (`idcargo`)
    REFERENCES `paperbook`.`Cargo` (`idcargo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_func_pk_endereco`
    FOREIGN KEY (`idendereco`)
    REFERENCES `paperbook`.`Endereco` (`idendereco`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_func_pk_contato`
    FOREIGN KEY (`idcontato`)
    REFERENCES `paperbook`.`Contato` (`idcontato`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_func_pk_usuario`
    FOREIGN KEY (`idusuario`)
    REFERENCES `paperbook`.`Usuario` (`idusuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Fornecedor` (
  ...
  INDEX `fk_fornecedor_idx` (`idendereco` ASC) ,
...
CREATE TABLE IF NOT EXISTS `paperbook`.`DetalhePedido` (
  ...
  INDEX `fk_detalhe_pk_pedido_idx` (`idpedido` ASC) ,
  INDEX `fk_detalhe_pk_produto_idx` (`idproduto` ASC) ,
  CONSTRAINT `fk_detalhe_pk_pedido`
    FOREIGN KEY (`idpedido`)
    REFERENCES `paperbook`.`Pedido` (`idpedido`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_detalhe_pk_produto`
